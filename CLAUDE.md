...
- **CI**: lint (`ruff check` + `ty check src`) and tests (matrix 3.9–3.14) run on PR/push to master, ignoring `docs/**`, `README.md`, `LICENSE`. Docs auto-deploy to GitHub Pages on EVERY push to master.
- **Dependabot** (`.github/dependabot.yml`): one grouped weekly PR bumping ONLY dev dependencies. `dependency-type: development` scopes updates to the PEP 735 `[dependency-groups]`; `[project.dependencies]` (fastapi, typing-extensions) are classified production and deliberately excluded so their intentional lower bounds stay put. `pre-commit`/`anyio`/`pytest` are `ignore`d above their last Python-3.9-compatible release (their 3.10-only latests can't resolve against `requires-python >=3.9`, which made Dependabot fail) — drop those ignores when 3.9 is dropped. It won't touch pre-commit hook revs — bump those with `pre-commit autoupdate`.
