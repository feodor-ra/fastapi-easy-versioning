...
- **Route scan** (`_iter_api_routes`, dual-path): on fastapi ≥0.137.2 routes are walked via the public `fastapi.routing.iter_route_contexts` (looked up with `getattr` at import — a static `from … import` would fail ty against older locked fastapi), yielding `_RouteView`s with **effective** path/methods/dependant (include-time prefixes and dependencies live only in the effective context since 0.137) plus the original route object; on pre-tree fastapi the old flat `isinstance(route, APIRoute)` scan is used (that branch is `# pragma: no cover` — locked fastapi is ≥0.137, it's exercised by the CI compat job).
- **Opt-in**: a route is "managed" only if its (effective) dependant has a dependency whose `.call` is a `VersioningSupport`; others are skipped and stay only in their own sub-app. Only `APIRoute` (HTTP) — WebSocket routes are not inherited.
