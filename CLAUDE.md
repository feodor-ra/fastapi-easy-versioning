...
- **CI**: lint (`ruff check` + `ty check src`) and tests (matrix 3.10–3.14) run on PR/push to master, ignoring `docs/**`, `README.md`, `README.ru.md`, `LICENSE`. A `fastapi-compat` job additionally runs the suite against fastapi `0.95.*` (minimum), `0.136.*` (last flat-router release) and latest via `uv run --frozen --with … -m pytest --no-cov`. Docs (`docs.yml`) build with `mkdocs build --strict` and deploy to GitHub Pages through the Pages Actions artifact (NOT `mkdocs gh-deploy`/the `gh-pages` branch — the Pages source is set to "GitHub Actions"), triggered by pushes to master touching `docs/**`, `overrides/**`, `mkdocs.yml`, `pyproject.toml`, `uv.lock`, `src/**` (docstrings feed the reference) or the workflow itself.
...
- **Docs are bilingual (EN + RU)** built with mkdocs-material + `mkdocs-static-i18n` + `mkdocstrings`. `docs/ru/*` is the source of truth; `docs/en/*` is a maintained first-class translation (no auto-translated banner) and is the **default** locale (English at `/`, Russian at `/ru/`). Mirror the same 10 pages in both trees: `index`, `quickstart`, `guide/{dependency,middleware,recipes,limitations}`, `examples/{simple,multiple}`, `reference/{dependency,middleware}`, `changelog`. There is now ONE `nav:` in `mkdocs.yml` plus `nav_translations` for ru — add a page to the single nav and translate its section title there.
- **Reference pages are generated** by mkdocstrings from the docstrings of the 6 public names (`::: fastapi_easy_versioning.X`), so a docstring is user-facing documentation: keep it Google-style and cross-link with `[`name`][fastapi_easy_versioning.name]`. Because the same identifiers are rendered in both locales, `autorefs.resolve_closest: true` is required — without it `--strict` aborts on "Multiple primary URLs".
- **`mkdocs build --strict` must stay clean** — it is what CI runs, and it catches broken internal links and unresolved mkdocstrings identifiers. Run it after moving or renaming any page.
- **Mermaid diagrams** use `classDef available/missing` + the `:::` operator (not per-node `style` lines). The classes carry an explicit `color:` — mermaid routes text properties of a class into the label's inline style, and without it Material paints the labels in the theme foreground, which is unreadable on the pastel fills in the slate scheme. The `pymdownx.superfences` custom fence for mermaid must stay in `mkdocs.yml`.
- **`check-yaml` runs with `--unsafe`** — mkdocs.yml carries `!!python/name:` tags the hook cannot construct.
- **`overrides/main.html`** exists solely to inject the Context7 chat widget into Material's `scripts` block (`extra_javascript` cannot carry the required `data-library` attribute). `context7.json` at the repo root drives Context7's indexing of `docs/en` — keep its `rules` in sync with the public API.
- **Drift risk**: the quick-start example is copied near-verbatim across `README.md`, `README.ru.md`, `docs/en/index.md`, `docs/ru/index.md`, `docs/*/quickstart.md`; the `docs/*/examples/*.md` code blocks are near-verbatim copies of `examples/*.py` (docs omit the in-file `# To run…` comment and use a clone-prefixed run command). Any public-API change must be propagated to all copies. Both READMEs use ABSOLUTE links — `README.md` is the PyPI long description, where relative links 404.
- **Release notes**: each release gets an entry in `docs/{en,ru}/changelog.md` (Keep a Changelog format) alongside the `chore: bump to X` commit.
